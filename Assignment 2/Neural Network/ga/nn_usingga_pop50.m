...
for it = 1:length(iter)
    ...
    for run = 1:max_runs
        ...
        options = gaoptimset ('TolFun',1e-8,'Generation',iter(it), 'PopInitRange', [-20;20],'PopulationSize', 50);
    ...
    save(['result_pop50_iter' num2str(iter(it)) '.mat']);
    sprintf('GA - population size 50 - iter %d : \n', iter(it))
